...
- 컬럼 수 제한: 표는 최대 3개 컬럼을 초과하지 마십시오. 비교 항목이 4개 이상이면 표를 분리하거나 일부 항목을 표 밖 텍스트로 설명하십시오.
- 법령 조항 금지: 표 셀 내부에 법령명, 조문 번호(예: 제○조, 국토계획법 §○ 등)를 포함하지 마십시오. 법령 근거는 선지별 해설에서 별도 기재합니다.
- 긴 설명 컬럼 분리 원칙: 하나의 셀 내용이 10자를 초과하는 서술형 텍스트라면, 해당 컬럼은 표에서 제거하고 표 위쪽에 별도 bullet 리스트로 기재하십시오. 표 셀에는 키워드(5~8자 내외)만 남기십시오.
